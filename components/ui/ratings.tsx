...
const Ratings = ({ onRatingClick, ...props }: RatingsProps) => {
  const {
    rating,
    totalStars = 5,
    size = 20,
    ...
    <div className={cn("flex items-center gap-2")} {...props}>
      ...
      {[...Array(totalStars - fullStars - (partialStar ? 1 : 0))].map((_, i) =>
        React.cloneElement(Icon, {
          key: i + fullStars + 1,
          size,
          className: cn("cursor-pointer", ratingVariants[variant].emptyStar),
          onClick: () => handleStarClick(i + fullStars + 1),
        }),
      )}
    </div>
  );
...
const PartialStar = ({ ...props }: PartialStarProps) => {
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      {React.cloneElement(Icon, {
        size,
        className: cn("fill-transparent cursor-pointer", className),
      })}
      <div
        style={{
          position: "absolute",
